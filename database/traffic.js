"use strict"
module.exports=(DB)=>{
function shift(a){a.shift();a.push(0);return a;}
...
const traffic={
    ...
    ins(sid){this._ins.run({sid,hs:JSON.stringify(new Array(24).fill(0)),ds:JSON.stringify(new Array(31).fill(0)),ms:JSON.stringify(new Array(12).fill(0))})},
    ...
    get(sid){
        ...
        this.ins(sid);
        return {hs:new Array(24).fill(0),ds:new Array(31).fill(0),ms:new Array(12).fill(0)};
    ...
    shift_ds(){
        for(var {sid,ds} of this.all())
            this.upd_hs(sid,shift(JSON.parse(ds)));
    },
    shift_ms(){
        for(var {sid,ms} of this.all())
            this.upd_hs(sid,shift(JSON.parse(ms)));
    }
}
